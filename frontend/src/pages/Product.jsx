...
function Product() {
  ...
  const handleAddToCart = () => {
    ...
    addToCart(product._id, selectedSize); // ← fixed: was onClick={addToCart} (passing click event as itemId)

    setAdded(true); // ← fixed: was never called, so green "✓ Added" state never showed
  ...
  const reviewCount = 122; // placeholder until reviews feature is built
    ...
    <>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .pp-left { flex-direction: row !important; }
          .pp-thumbs { flex-direction: row !important; width: auto !important; }
          .pp-thumb-item { width: 64px !important; flex-shrink: 0; }
        }
      `}</style>
      ...
      >
        ...
        >
          ...
          <div
            className="pp-left"
            style={{ display: "flex", gap: 14, flex: "0 0 auto", width: "55%" }}
          >
            {/* Thumbnail strip */}
            <div
              className="pp-thumbs"
            ...
            >
              ...
              {!inStock && (
                ...
                >
                  <span style={{ fontSize: 18, color: "#555", fontWeight: 500 }}>
                    Out of Stock
                  </span>
                </div>
              )}
          ...
          <div style={{ flex: 1, paddingTop: 4 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                <p key={line} style={{ fontSize: 13, color: "#666", margin: 0 }}>
                  {line}
                </p>
              ))}
            </div>
        ...
        <div style={{ marginTop: 56 }}>
          ...
          >
            <button
              className={`pp-tab-btn${activeTab === "description" ? " active" : ""}`}
              onClick={() => setActiveTab("description")}
            >
              Description
            </button>
            <button
              className={`pp-tab-btn${activeTab === "reviews" ? " active" : ""}`}
            ...
            <div style={{ maxWidth: 720 }}>
              ...
              {product.subCategory && (
                <p style={{ fontSize: 13, color: "#888", marginTop: 12 }}>
                  Type:{" "}
                  <strong style={{ color: "#555" }}>{product.subCategory}</strong>
                </p>
              )}
              {product.brand && (
                <p style={{ fontSize: 13, color: "#888", marginTop: 6 }}>
                  Brand:{" "}
                  <strong style={{ color: "#555" }}>{product.brand}</strong>
                </p>
              )}
              <p style={{ fontSize: 13, color: "#888", marginTop: 6 }}>
                Stock:{" "}
                <strong style={{ color: "#555" }}>{product.stock} units</strong>
              </p>
            </div>
          ) : (
            <div style={{ maxWidth: 720 }}>
